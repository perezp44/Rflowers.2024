...
subtitle: | 
  (Aún en elaboración) Un análisis con datos de Wikidata sobre la ubicación de los cuadros de Goya en el mundo.
date: 2024-03-25
...
Veamos algunas tabla. Primero una tabla con el núemro de Goyas en cada museo:
...
zz_museos <- dplyr::left_join(zz_unir_museos, zz_museos) %>% dplyr::arrange(desc(n))
```


Las tabla están hechas pero no se muestran. Aún no se el perquè
...
- las DT y recatable no se ven, asi que provisionalmente una gt::gt()

```{r}
gt::gt(zz_museos)
```

Faltan las tablas por ciudades y por paises . Están hechas pero se acab el tiempo 
